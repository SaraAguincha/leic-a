...
public class WeeklyScore implements DomainEntity {
    ...
    public void computeStatistics() {
        ...
        if (LocalDateTime.now().isAfter(week.plusDays(7).atStartOfDay())) {
    ...
    public void setClosed(boolean close) {
        ...
        LocalDate currentWeek = LocalDateTime.now().with(weekSunday).toLocalDate();
        if (close && week.isEqual(currentWeek)) {
            throw new TutorException(CANNOT_CLOSE_CURRENT_WEEK, LocalDateTime.parse(week.atStartOfDay().toString()).toString());
        }
