...
Cypress.Commands.add('solveQuizzFailedAnswers', (quizTitle, numberOfQuizQuestions) => {
  cy.get('[data-cy="quizzesStudentMenuButton"]').click();
  cy.contains('Available').click();
  ...
  cy.route('POST', '/answers/*/conclude').as('concludeQuiz');
  cy.get('[data-cy="endQuizButton"]').click();
  cy.get('[data-cy="confirmationButton"]').click();
  cy.wait('@concludeQuiz');
});

Cypress.Commands.add('updateAndDeleteFailedAnswers', () => {
  cy.route('GET', '/students/dashboards/executions/*').as('getDashboard');
  cy.route('GET', '/failedAnswers/dashboard/*').as('getFailedAnswers');
  cy.route('PUT', '/students/dashboard/*/failedAnswers').as('updateFailedAnswers');
  cy.route('DELETE', '/failedAnswers/*/delete').as('deleteFailedAnswer');

  cy.get('[data-cy="dashboardMenuButton"]').click();
  cy.wait('@getDashboard').its('status').should('eq', 200);

  cy.get('[data-cy="failedAnswersMenuButton"]').click();
  cy.wait('@getFailedAnswers').its('status').should('eq', 200);

  cy.get('[data-cy="refreshFailedAnswers"]').click();
  cy.wait('@updateFailedAnswers').its('status').should('eq', 200);

  cy.get('[data-cy="showStudentViewDialog"]').eq(0).click({force: true});
  cy.get('[data-cy="closeButton"]').click();

  cy.get('[data-cy="deleteFailedAnswerButton"]').eq(0).click();
  cy.on('window:confirm', () => true);
  cy.wait('@deleteFailedAnswer').its('status').should('eq', 400);
  ...
  cy.get('[data-cy="deleteFailedAnswerButton"]').eq(0).click();
  cy.on('window:confirm', () => true);
  cy.wait('@deleteFailedAnswer').its('status').should('eq', 200);
});

Cypress.Commands.add('solveQuizzDifficultQuestions', (quizTitle, numberOfQuizQuestions) => {
    cy.get('[data-cy="quizzesStudentMenuButton"]').click();
    cy.contains('Available').click();
...
Cypress.Commands.add('updateAndDeleteDifficultQuestions', () => {
    cy.route('GET', '/students/dashboards/executions/*').as('getDashboard');
    cy.route('GET', '/difficultQuestions/dashboard/*').as('getDifficultQuestions');
    cy.route('PUT', '/difficultQuestions/dashboard/*').as('updateDifficultQuestions');
    cy.route('DELETE', '/difficultQuestions/*/delete').as('deleteDifficultQuestion');

    cy.get('[data-cy="dashboardMenuButton"]').click();
    cy.wait('@getDashboard').its('status').should('eq', 200);

    cy.get('[data-cy="difficultQuestionsMenuButton"]').click();
    cy.wait('@getDifficultQuestions').its('status').should('eq', 200);

    cy.get('[data-cy="refreshDifficultQuestions"]').click();
    cy.wait('@updateDifficultQuestions').its('status').should('eq', 200);

    cy.get('[data-cy="showStudentViewDialog"]').eq(0).click({force: true});
    cy.get('[data-cy="closeButton"]').click();

    cy.get('[data-cy="deleteDifficultQuestionButton"]').eq(0).click();
    cy.on('window:confirm', () => true);
    cy.wait('@deleteDifficultQuestion').its('status').should('eq', 200);
...
Cypress.Commands.add('solveQuizzWeeklyScores', (quizTitle, numberOfQuizQuestions) => {
  cy.get('[data-cy="quizzesStudentMenuButton"]').click();
  cy.contains('Available').click();
...
Cypress.Commands.add('updateAndDeleteWeeklyScores', () => {
  cy.route('GET', '/students/dashboards/executions/*').as('getDashboard');
  cy.route('GET', '/students/dashboards/*/weeklyscores').as('getWeeklyScores');
  cy.route('PUT', '/students/dashboards/*/weeklyscores').as('updateWeeklyScores');
  cy.route('DELETE', '/students/weeklyscores/*').as('deleteWeeklyScores');
  ...
  cy.get('[data-cy="refreshWeeklyScores"]').click();
  cy.wait('@updateWeeklyScores');

  cy.get('[data-cy="deleteWeeklyScoreButton"]').should('have.length', 2).eq(1).click();
  cy.on('window:confirm', () => true);
  cy.wait('@deleteWeeklyScores');

  cy.get('[data-cy="deleteWeeklyScoreButton"]').should('have.length', 1).eq(0).click();
  cy.on('window:confirm', () => true);
  cy.wait('@deleteWeeklyScores').its('status').should('eq', 400);
  cy.closeErrorMessage();
