...
export default class RemoteServices {
  ...
  static async updateDifficultQuestion(dashboardId: number){
    return httpClient
        .put(`/difficultQuestions/dashboard/${dashboardId}`)
        .catch(async (error) => {
          throw Error(await this.errorMessage(error));
        });
  }
  ...
  static async deleteDifficultQuestion(difficultQuestionId: number){
    return httpClient
        .delete(`/difficultQuestions/${difficultQuestionId}/delete`)
        .catch(async (error) => {
          throw Error(await this.errorMessage(error));
        });
  }

  // Failed Answer Controller

  static async getFailedAnswers(
    dashboardId: number
  ): Promise<FailedAnswer[]> {
    return httpClient
      .get(
        `/failedAnswers/dashboard/${dashboardId}`
      )
      .then((response) => {
  ...
  static async updateFailedAnswers(
    ...
    return httpClient
      .put(
        `/students/dashboard/${dashboardId}/failedAnswers`
      )
      .then((response) => {
        return response.data.map((failedAnswer: any) => {
          return new FailedAnswer(failedAnswer);
  ...
  static async deleteFailedAnswer(
    failedAnswerId: number
  ) {
    return httpClient
      .delete(
        `/failedAnswers/${failedAnswerId}/delete`
      )
      .catch(async (error) => {
        throw Error(await this.errorMessage(error));
      });
  ...
  static async updateWeeklyScore(dashboardId: number) {
    return httpClient
