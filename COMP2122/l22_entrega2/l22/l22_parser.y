...
%type <lvalue> lvalue
%type <i> qualifier
%type <type> data_type var_type func_type
%type <block> block
%type <s> string
...
file                : /* empty */                      { compiler->ast($$ = new cdk::sequence_node(LINE)); }
                    | opt_variable_decls               { $$ = $1; }
                    | file program                     { $$ = new cdk::sequence_node(LINE, $2, $1); } /* not sure about this*/
                    ;
...
var_type            : tVAR                             { $$ = cdk::primitive_type::create(0, cdk::TYPE_UNSPEC); }
                    ;
...
variable_decls      : variable_decl        ';'                                 { $$ = new cdk::sequence_node(LINE, $1);     }
                    | variable_decl        ';'  variable_decls                 { $$ = new cdk::sequence_node(LINE, $1, $3); }
                    | variable_decl_block                                      { $$ = new cdk::sequence_node(LINE, $1);     }
                    | variable_decl_block       variable_decls                 { $$ = new cdk::sequence_node(LINE, $1, $2); }
                    ;
...
data_type           : tINT_TYPE                        { $$ = cdk::primitive_type::create(4, cdk::TYPE_INT);     }
                    ...
                    | '[' data_type ']'                { $$ = cdk::reference_type::create(4, std::shared_ptr<cdk::basic_type>($2)); }
                    | '[' tVOID_TYPE ']'               { $$ = cdk::reference_type::create(4, cdk::primitive_type::create(4, cdk::TYPE_VOID)); }
                    | func_type                        { $$ = $1; }
                    ;

data_types          :                data_type         { $$ = std::vector<std::shared_ptr<cdk::basic_type>> {$1}; }
...
func_def            : '(' ')' '-' '>'      data_type  ':' block    { $$ = new l22::function_definition_node(LINE, $5, nullptr, $7); }
                    | '(' args ')' '-' '>' data_type  ':' block    { $$ = new l22::function_definition_node(LINE, $6, $2, $8);      }
                    | '(' ')' '-' '>'      tVOID_TYPE ':' block    { $$ = new l22::function_definition_node(LINE, cdk::primitive_type::create(4, cdk::TYPE_VOID), nullptr, $7); }
                    | '(' args ')' '-' '>' tVOID_TYPE ':' block    { $$ = new l22::function_definition_node(LINE, cdk::primitive_type::create(4, cdk::TYPE_VOID), $2, $8);      }
                    ;

func_type           : data_type '<' data_types '>'     { $$ = cdk::functional_type::create($3, $1); }
...
instructions        : instruction                      { $$ = new cdk::sequence_node(LINE, $1);     }
                    ...
                    | instruction  ';'  instructions   { $$ = new cdk::sequence_node(LINE, $1, $3); }
                    | block_instruction instructions   { $$ = new cdk::sequence_node(LINE, $1, $2); }
                    ;
