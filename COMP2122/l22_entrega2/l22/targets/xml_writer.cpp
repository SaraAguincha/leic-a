...
void l22::xml_writer::do_continue_node(l22::continue_node * const node, int lvl) {
  openTag(node, lvl);
  closeTag(node, lvl);
}
...
void l22::xml_writer::do_function_call_node(l22::function_call_node *const node, int lvl) {
  os() << std::string(lvl, ' ') << "<" << node->label() << " expression='" << node->expression() << "'>" << std::endl;
...
void l22::xml_writer::do_print_node(l22::print_node * const node, int lvl) {
  ...
  //node->newline()->accept(this, lvl + 4);
